...
{ \
  ...
  echo "set -x"; \
  echo "echo START REQ"; \
  echo "cat - | openssl aes-256-cbc -pass pass:${PASSWORD} -base64 | curl -v -NsS https://ppng.io/${KEYWORD}req"; \
} >./req.sh
...
{ \
  ...
  echo "set -x"; \
  echo "echo START RES"; \
  echo "curl -v -m 3600 -NsST - https://ppng.io/${KEYWORD}res | openssl aes-256-cbc -d -pass pass:${PASSWORD} -base64"; \
} >./res.sh
