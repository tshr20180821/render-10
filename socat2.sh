...
if [ ! -z "${X_GD_APIKEY}" ]; then
  URL_RES=$(curl -sS "https://xgd.io/V1/shorten?url=$(${PIPING_SERVER}/${KEYWORD}res | jq -Rr @uri)&key=${X_GD_APIKEY}&analytics=false" | jq '.shorturl')
  URL_REQ=$(curl -sS "https://xgd.io/V1/shorten?url=$(${PIPING_SERVER}/${KEYWORD}req | jq -Rr @uri)&key=${X_GD_APIKEY}&analytics=false" | jq '.shorturl')
fi
