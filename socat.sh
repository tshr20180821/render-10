...
do
  ...
  socat -ddd "exec:curl --http1.1 -NsS ${PIPING_SERVER}/${KEYWORD}req!!exec:curl --http1.1 -m 3600 -NsST - ${PIPING_SERVER}/${KEYWORD}res" \
  ...
  #   tcp:127.0.0.1:${TARGET_PORT}
  socat -ddd "exec:curl -NsS ${PIPING_SERVER}/${KEYWORD}req!!exec:curl -m 3600 -NsST - ${PIPING_SERVER}/${KEYWORD}res" \
    tcp4:127.0.0.1:${TARGET_PORT}
done
